...
public class StreamGobbler {
  ...
  public static void streamAndStop(
      Process process,
      int waitTimeoutBeforeStopMs,
      int waitStopStreamsTimeoutMs,
      Consumer<String> infoHandler,
      Consumer<String> errorHandler) {
    StreamGobbler killStreamGobbler =
        new StreamGobbler("kill-sig-" + process.pid(), infoHandler, errorHandler);
  ...
  public void stream(Process process) {
    errorFuture =
        executorService.submit(
            new StreamReader(
                name + "/error stream reader", process.getErrorStream(), errorConsumer));
    inputFuture =
        executorService.submit(
            new StreamReader(
                name + "/input stream reader", process.getInputStream(), inputConsumer));
  }
